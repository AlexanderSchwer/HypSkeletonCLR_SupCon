...
class CrosSCLR(nn.Module):
    ...
    def forward(self, im_q, im_k=None, view='all', cross=False, topk=1, context=True): 
        ...
        # compute logits
        # positive logits: Nx1
        # HYP: Replace dot product with (negative) hyperbolic distance
        ...
        l_neg = -poincare_ball.dist(q_expanded, queue_expanded)  # Shape (N, K)
        ...
        # Compute all pairwise (negative) hyperbolic distances between q and queue
        l_neg_bone = -poincare_ball.dist(q_bone_expanded, queue_bone_expanded)  # Shape (N, K)
    ...
    def cross_training(self, im_q, im_k, topk=1, context=True):
        ...
        # HYP: Replace dot product with hyperbolic distance
        l_pos = -poincare_ball.dist(q, k).unsqueeze(-1)
        ...
        q_expanded = q.unsqueeze(1)             # Shape (N, 1, C)
        queue_expanded = queue.unsqueeze(0)     # Shape (1, K, C)
        # Compute all pairwise hyperbolic distances between q and queue
        l_neg = -poincare_ball.dist(q_expanded, queue_expanded)  # Shape (N, K)
